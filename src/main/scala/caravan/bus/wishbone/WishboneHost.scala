...
class WishboneHostIO(implicit val config: WishboneConfig) extends HostAdapterIO {
  val masterTransmitter = Decoupled(new WishboneMaster())
  val slaveReceiver = Flipped(Decoupled(new WishboneSlave()))
...
class WishboneHost(implicit val config: WishboneConfig) extends HostAdapter {
  val io = IO(new WishboneHostIO)

  def getAddressPin: UInt = io.masterTransmitter.bits.adr


  def fire(): Bool = io.reqIn.valid && io.masterTransmitter.ready
  /**
   ...
   */
  io.masterTransmitter.valid := io.masterTransmitter.bits.stb

  /** FIXME: Assuming Master is always ready to accept data from Slave */
  io.slaveReceiver.ready := true.B
  dontTouch(io.masterTransmitter.ready)
  dontTouch(io.slaveReceiver.ready)
  ...
  when(reset.asBool() === true.B) {
     ...
     */
    io.masterTransmitter.bits.getElements.filter(w => DataMirror.directionOf(w) == ActualDirection.Output).map(_ := 0.U)
  }
  ...
  val ackReg = RegInit(io.slaveReceiver.bits.ack)
  ...
  if(!config.waitState) {
    ...
    io.masterTransmitter.bits.stb := stbReg
    io.masterTransmitter.bits.cyc := cycReg
    io.masterTransmitter.bits.we := weReg
    io.masterTransmitter.bits.adr := adrReg
    io.masterTransmitter.bits.dat := datReg
    io.masterTransmitter.bits.sel := selReg

    when(!startWBTransaction) {
      io.masterTransmitter.bits.getElements.filter(w => DataMirror.directionOf(w) == ActualDirection.Output).map(_ := 0.U)
    }

    when(io.slaveReceiver.bits.ack && !io.slaveReceiver.bits.err) {
      dataReg := io.slaveReceiver.bits.dat
      respReg := true.B
      errReg := false.B
    ...
    } .elsewhen(io.slaveReceiver.bits.err && !io.slaveReceiver.bits.ack) {
      dataReg := io.slaveReceiver.bits.dat
      respReg := true.B
      errReg := true.B
      startWBTransaction := false.B
    ...
    when(stateReg === idle) {
      stateReg := Mux(io.slaveReceiver.bits.ack || io.slaveReceiver.bits.err, latch_data, idle)
