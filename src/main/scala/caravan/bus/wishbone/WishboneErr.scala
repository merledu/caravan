...
class WishboneErrIO(implicit val config: WishboneConfig) extends ErrorDeviceIO {
  val slaveTransmitter = Decoupled(new WishboneSlave())
  val masterReceiver = Flipped(Decoupled(new WishboneMaster()))
}

class WishboneErr(implicit val config: WishboneConfig) extends ErrorDevice {
  val io = IO(new WishboneErrIO)
  ...
  io.slaveTransmitter.bits.ack := ackReg
  io.slaveTransmitter.bits.dat := dataReg
   io.slaveTransmitter.bits.err := errReg
